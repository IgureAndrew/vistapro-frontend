// check_forms_correct.js
// Check if today's marketers have completed their forms (CORRECT VERSION)
...
async function checkForms() {
  try {
    ...
    for (const verification of verifications.rows) {
      ...
      // Check guarantor form using marketer_id
      ...
      // Check commitment form using marketer_id
    ...
    for (const verification of verifications.rows) {
      ...
      const guarantor = await pool.query('SELECT id FROM marketer_guarantor_form WHERE marketer_id = $1', [verification.marketer_id]);
      const commitment = await pool.query('SELECT id FROM marketer_commitment_form WHERE marketer_id = $1', [verification.marketer_id]);
