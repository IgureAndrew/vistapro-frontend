// check_marketer_forms_completion.js
// Check if marketers have completed their biodata, guarantor, and commitment forms
...
async function checkMarketerFormsCompletion() {
  try {
    console.log('🔍 Checking marketer forms completion status...\n');
    ...
    console.log(`📊 Found ${verifications.rows.length} verification submission(s)\n`);
    ...
    for (const verification of verifications.rows) {
      ...
      const biodataQuery = `
        ...
        LIMIT 1;
      `;
      const biodata = await pool.query(biodataQuery, [verification.marketer_id]);
      ...
      const guarantorQuery = `
        ...
        LIMIT 1;
      `;
      const guarantor = await pool.query(guarantorQuery, [verification.marketer_id]);
      ...
      const commitmentQuery = `
        ...
        LIMIT 1;
      `;
      const commitment = await pool.query(commitmentQuery, [verification.marketer_id]);
    ...
    for (const verification of verifications.rows) {
      const biodata = await pool.query('SELECT id FROM marketer_biodata WHERE user_id = $1', [verification.marketer_id]);
      const guarantor = await pool.query('SELECT id FROM marketer_guarantor_form WHERE user_id = $1', [verification.marketer_id]);
      const commitment = await pool.query('SELECT id FROM marketer_commitment_form WHERE user_id = $1', [verification.marketer_id]);
