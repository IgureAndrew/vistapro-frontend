...
async function testSuperAdminOrders() {
  try {
    console.log('🔍 Testing SuperAdmin orders query...');
    
    // First, get Andu Eagle's ID
    const superAdminResult = await pool.query(`
      SELECT id, unique_id, first_name, last_name
      FROM users 
      WHERE unique_id = 'SM000005' AND role = 'SuperAdmin'
    `);
    ...
    console.log(`✅ Found SuperAdmin: ${superAdmin.first_name} ${superAdmin.last_name} (ID: ${superAdmin.id})`);
    ...
    const query = `
      ...
      ORDER BY o.sale_date DESC LIMIT 100
    `;
    
    const result = await pool.query(query, [superAdmin.id]);
    
    console.log(`\n📊 Found ${result.rows.length} orders for SuperAdmin ${superAdmin.first_name} ${superAdmin.last_name}:`);
    ...
    const hierarchyResult = await pool.query(hierarchyQuery, [superAdmin.id]);
    
    console.log(`\n👥 Marketers under ${superAdmin.first_name} ${superAdmin.last_name}:`);
