...
export default function MasterAdminOverview() {
  ...
  <table className="min-w-full divide-y divide-gray-200">
    <thead className="bg-gray-50">
      <tr>
        {["Activity", "Entity", "Entity ID", "Performed By", "When"].map((hdr) => (
          <th
            key={hdr}
    ...
    <tbody className="divide-y divide-gray-200">
      {recentActivity.length > 0 ? (
        recentActivity.map((act) => (
          <tr key={act.id}>
            <td className="px-4 py-2 text-sm text-gray-700">{act.activityType}</td>
            <td className="px-4 py-2 text-sm text-gray-700">{act.entityType}</td>
            <td className="px-4 py-2 text-sm text-gray-700">{act.entityId}</td>
            <td className="px-4 py-2 text-sm text-gray-700">{act.actorName}</td>
