...
const updateAccount = async (req, res) => {
  try {
    ...
    if (req.file) {
      ...
      try {
        console.log('🖼️ Uploading file to Cloudinary for SuperAdmin:', userId);
      ...
      try {
        console.log('🖼️ Uploading Base64 image to Cloudinary for SuperAdmin:', userId);
    ...
    const result = await pool.query(updateQuery, [email, phone, firstName, lastName, profileImageData, gender, ...passwordParams, userId]);
