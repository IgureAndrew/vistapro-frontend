              ...
              {selectedProductId && (
                <div className="md:col-span-2">
                  ...
                  <textarea
                    readOnly
                    rows={4}
                    value={dealerProducts
                      .find(p => p.product_id === +selectedProductId)
                      ?.imeis_available.slice(0, Number(form.number_of_devices) || 0)
                      .join("\n")}
        ...
        <div className="sm:hidden space-y-4">
          {orders.length ? orders.map(o => (
            <div key={o.id} className="bg-white p-4 rounded-lg shadow">
              ...
              <p className="text-sm"><strong>Device:</strong> {o.device_name} {o.device_model}</p>
              <p className="text-sm"><strong>Qty:</strong> {o.number_of_devices}</p>
              <p className="text-sm"><strong>Amount:</strong> ₦{Number(o.sold_amount).toLocaleString()}</p>
              <p className="text-xs text-gray-500"><strong>Date:</strong> {new Date(o.sale_date).toLocaleString()}</p>
            </div>
          )) : (
              ...
              <thead className="bg-gray-50">
                <tr>
                  {["#","Device","Model","Qty","Amount","Date","Status"].map((h,i) => (
                    <th key={i} className="px-3 py-2 text-left font-semibold uppercase">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-gray-200">
                {orders.map(o => (
                  <tr key={o.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-3 py-2">{o.device_name}</td>
                    <td className="px-3 py-2">{o.device_model}</td>
                    <td className="px-3 py-2">{o.number_of_devices}</td>
                    <td className="px-3 py-2">₦{Number(o.sold_amount).toLocaleString()}</td>
                    <td className="px-3 py-2">{new Date(o.sale_date).toLocaleDateString()}</td>
