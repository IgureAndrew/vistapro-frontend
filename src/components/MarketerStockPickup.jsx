...
export default function MarketerStockPickup() {
  ...
  async function onSubmit(data) {
    ...
    try {
      const qty = Math.min(data.quantity, maxQty);
  ...
  async function onRequestSubmit() {
    if (!requestProduct || requestQty < 2 || requestQty > 3) {
      alert("Select a product and request 2 or 3 units.");
      return;
    }
    try {
      await api.post("/stock/request", {
        ...
        <div className="grid gap-4">
          ...
          {selectedProd && (
            <div>
              ...
              {maxQty === 1 ? (
                <div className="px-3 py-2 border rounded bg-gray-100">1</div>
              ) : (
                <input
                  type="number"
                  min="1"
          ...
          {requesting && (
            <div className="space-y-2">
              <h3 className="font-medium">Request More Stock</h3>
              <select
              ...
              <input
                ...
                className="w-full border rounded px-3 py-2"
                placeholder="Quantity (2-3)"
              />
              <div className="flex gap-2">
                <button onClick={onRequestSubmit} className="flex-1 bg-green-500 text-white py-1 rounded">
                  Submit
                </button>
                <button onClick={() => setRequesting(false)} className="flex-1 bg-gray-300 py-1 rounded">
                  Cancel
                </button>
              </div>
            </div>
              ...
              {pickups.length === 0 ? (
                ...
                return (
                  <tr key={s.id} className="hover:bg-gray-50">
                    <td className="px-4 py-2">{s.device_name} {s.device_model}</td>
