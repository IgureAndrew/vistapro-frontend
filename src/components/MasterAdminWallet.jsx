...
export default function MasterAdminWallet() {
  ...
  // ── Summary Cards ─────────────────────────────────────────────
  const total_balance     = wallets.reduce((s, w) => s + Number(w.total_balance || 0), 0)
  const available_balance = wallets.reduce((s, w) => s + Number(w.available_balance || 0), 0)
  const withheld_balance  = wallets.reduce((s, w) => s + Number(w.withheld_balance || 0), 0)
  const pending_cashout   = pendingReqs
    .filter(r => r.status === 'pending')
    .reduce((s, r) => s + ((Number(r.amount_requested) - Number(r.fee)) || 0), 0)
    ...
    <div className="p-6 space-y-8">
        ...
        {pendingReqs.length === 0 ? (
          ...
          <table className="min-w-full text-sm text-left">
            <thead className="bg-gray-100">
              <tr>
                {['ID','User','Requested','Fee','Net','Account Name','Account Number','Bank Name','Date','Actions']
              ...
              {pendingReqs.map(r => {
                ...
                return (
                  <tr key={r.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2">₦{requested.toLocaleString()}</td>
      ...
      <div className="bg-white rounded-2xl shadow p-6 overflow-x-auto">
        ...
        <form onSubmit={applyFilters} className="grid grid-cols-1 md:grid-cols-6 gap-4 mb-4">
          ...
          <input type="text" name="name" placeholder="Name" value={filters.name} onChange={handleNameChange} className="border rounded p-2" />
